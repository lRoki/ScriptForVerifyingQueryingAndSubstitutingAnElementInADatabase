...
def get_contract(limit=None):
    ...
    sql = f'SELECT Contrato FROM usuarios WHERE usuario = "Flamex" AND Situacao = "Não localizado"{limit}'
    cursor.execute(sql)
...
def main():
    ...
    rows = get_contract()
##    for row in rows:
##        print(f"Longitud: {row[0]} - {len(row[0])} - with trim {len(row[0].strip())}")
##    print(rows)
##        ConsultaSituacaoEmprestimo
##    situacao = client.service.ConsultaSituacaoProposta(sLogin,sSenah,"23397020	")
##    situacao = client.service.ConsultaSituacaoProposta(sLogin,sSenah,72664249)
##    print(situacao)   
##    input()
    ...
    if rows:
        for contract in rows:
            contract_number = contract[0].strip()
